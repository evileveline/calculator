...
class NumberService {
    ...
    static Number parseAndValidate(String symbol) throws Exception {
        ...
        if (value < 1 || value > 10) {
            throw new Exception("Ошибка: калькулятор работает только с числами от 1 до 10 включительно.");
        }
    ...
    private static int letterToNumber(char letter) throws Exception {
    ...
    static int toArabicNumber(String roman) throws Exception {
        ...
        while (i < roman.length()) {
            ...
            if (num < 0) throw new Exception("Ошибка: неверный римский символ");
